...
$loader->registerDefinition("CircleService", $GEN_DIR);
$loader->register();
...
try {
    ...
    $socket = new TSocket("172.16.0.3",9090);
    //$socket = new THttpClient('127.0.0.1', 80,'/init/phpServer.php');
    $transport = new TBufferedTransport($socket);
    $protocol = new TBinaryProtocol($transport);
    //$client = new \tService\UserServiceClient($protocol);
    $client = new \CircleService\CircleServiceClient($protocol);
    $transport->open();
   
    $res = $client->getFriends('l35789637007221180030');
    echo $res;
